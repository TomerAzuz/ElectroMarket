...
helm repo update
helm upgrade --install electro-keycloak bitnami/keycloak \
  --values values.yml \
...
echo "Admin Password: $(kubectl get secret --namespace keycloak-system electro-keycloak -o jsonpath="{.data.admin-password}" | base64 --decode)"

echo "\n🔑 Generating Secret with Keycloak client secret."

kubectl delete secret electro-keycloak-client-credentials || true

kubectl create secret generic electro-keycloak-client-credentials \
