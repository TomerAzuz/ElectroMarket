  ...
  return (
    <div className='flex gap-x-4 py-2 lg:px-6 border-b border-gray-200 w-full font-light text-gray-500'>
      <div className='w-full min-h-[150px] flex items-center gap-x-4'>
        <Link to={`products/${id}`}>
          <img className='max-w-[80px]' src={imageUrl} alt={name}/>
        </Link>
        <div className='w-full flex flex-col'>
          ...
          <div className='flex gap-x-2 h-[36px] text-sm'>
            ...
            h-full border text-primary font-medium'>
              ...
              <div onClick={() => increaseQuantity(id)} className='flex-1 h-full flex justify-center 
            ...
            text-primary font-medium'>{`$ ${parseFloat(price * quantity).toFixed(2)}`}</div>
          </div>
        </div>
      </div>
    </div>
